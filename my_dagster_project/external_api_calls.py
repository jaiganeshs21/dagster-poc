...
from everstage_api_task import commission_task_to_everstage, databook_task_to_everstage, task_status_checker
import time
...
def submit_databook_sync_task():
    ...
    task_status = get_task_status(job_id=job_id, timeout=1000, interval=5, task_name="DATASHEET")
...
def submit_commission_sync_task(databook_task):
    ...
    task_status = get_task_status(job_id=job_id, timeout=1000, interval=5, task_name="COMMISSION")
    if(task_status == "complete"):
